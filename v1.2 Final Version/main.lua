...
function love.load()
    ...
    sounds = {
        ...
        balloonPop = love.audio.newSource("sounds/balloonPop.mp3", "static"),
    ...
    sounds.laser:setPitch(2)

    ------------------------ particle-system ---------------------------------
    particleImage = love.graphics.newImage("textures/smoke.png")
    psystem = love.graphics.newParticleSystem(particleImage, 1000)
    psystem:setParticleLifetime(2, 5) -- Particles live at least 2s and at most 5s.
	psystem:setEmissionRate(250)
	psystem:setSizeVariation(1)
	psystem:setLinearAcceleration(-20, -3, 0, 3)
	psystem:setColors(167/255, 226/255, 206/255, 0.1, 43/255, 151/255, 136/255, 1, 0, 1, 1, 0) -- Fade to transparency.
    psystem:setSpeed(1, 2)
    psystem:setSizes(0.5,0.2,0.1)
    ---------------------------------------------------------------------------
...
function love.draw()
    ...
    if game.state["menu"] then
        ...
        buttons.menu_state.settings:draw(love.graphics.getWidth() / 2 - 125, love.graphics.getHeight() / 2 - 60 + 80, 24, 12, 0.91, 0.50, 0.30)
        buttons.menu_state.exit_game:draw(love.graphics.getWidth() / 2 - 125, love.graphics.getHeight() / 2 - 60 + 160, 20, 12, 0.91, 0.50, 0.30)
    end
    ...
    if game.state["settings"] then 
        ...
        buttons.settings_state.menu:draw(20, 20, 20, 12, 0.91, 0.50, 0.30)
    end
    ...
    if game.state["running"] then
        ...
        love.graphics.printf("Timer: "..math.floor(timer), love.graphics.newFont(24), 0, 10, love.graphics.getWidth(), "center") --Timer
    end
    
    ----------------------------------- particle system ---------------------------------------
    if game.state["running"] then
        love.graphics.draw(psystem, player.x+70, player.y+140)
    end
    -------------------------------------------------------------------------------------------
end
        ...
        if button == 1 then
            ...
            elseif game.state["settings"] then
                for index in pairs(buttons.settings_state) do
                    buttons.settings_state[index]:checkPressed(mouse.x, mouse.y)
                    sounds.button:play()
                end
            end
        end
...
function gameStateAndButtons()
    ...
    buttons.menu_state.settings = button("Settings", changeGameState, "settings", 250, 60)

    buttons.settings_state.menu = button("Menu", changeGameState, "menu", 150, 60)
